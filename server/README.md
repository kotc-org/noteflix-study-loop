...
Required environment variables are `REVIEWER_SECRET_NAME`, `MCP_TRANSPORT_ORIGIN`, and `MCP_PUBLIC_ORIGIN`. Fixture mode additionally requires `FIXTURE_SECRET_NAME`, `FOREIGN_REVIEWER_SECRET_NAME`, and the exact confirmation string enforced by the script. The reviewer/foreign-reviewer pair is fail-closed to the two dedicated review accounts, and the foreign video must be the exact video recorded in the counterpart fixture secret under that exact Firebase UID. Never place reviewer credentials, tokens, fixture IDs, or the fixture-secret payload in shell history, logs, source control, or CI output.

When production has no known-good video owned by a separate test account, `scripts/prepare-cross-account-reviewer.mjs` can create the fixed locked synthetic Firebase account, store its random credential in Secret Manager, and pre-provision a 60-day Noteflix Pro reviewer entitlement without a purchase. The entitlement grant is pinned to the immutable production RevenueCat project and entitlement IDs. The script is guarded by its own exact confirmation value, is idempotent on later runs, and prints only booleans plus a truncated UID hash. Run the production fixture verifier with that credential and a separate fixture secret; never use a real learner or personal subscription for the denial fixture.
