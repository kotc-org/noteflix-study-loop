...
- [ ] The app's general-audience educational purpose and restricted-data refusal behavior are tested.
...
- [ ] `create_public_note_video` has `readOnlyHint: false`, `destructiveHint: false`, `openWorldHint: true`, and `idempotentHint: true`; all three explicit confirmation fields are enforced server-side.
...
- [ ] Portal CSP is explicit and deny-all for this no-component release: `connectDomains: []`, `resourceDomains: []`, and `frameDomains: []`.
...
- [ ] Restricted-data rejection runs before eligibility, idempotency, allowance, moderation, or generation-provider calls and never echoes the matched value or category.
...
- [ ] Exactly five positive and three negative scenarios from `REVIEW_SCENARIOS.md` pass against production on both ChatGPT web and mobile, and every expected field name matches the deployed snake_case contract.
