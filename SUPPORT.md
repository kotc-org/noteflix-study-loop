...
The four study skills require no Noteflix account. Saving a private note requires a Noteflix account and an OAuth connection; learners do not need an API key or environment variable.
...
3. For a save problem, confirm that Noteflix is connected through OAuth and that the `notes:create` permission was granted.
